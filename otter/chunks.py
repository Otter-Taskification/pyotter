...
class Chunk:
    ...
    def as_graph(self, verbose: bool=False):
        ...
        # Also require at least 1 edge for single-executor chunks if there are no edges currently
        # task chunks
        if self.kind != RegionType.explicit_task and len(self.events) <= 2 and g.ecount() == 0:
            g.add_edge(g.vs[0], g.vs[1])
        elif self.kind == RegionType.single_executor and g.vertex_connectivity(root_node.index, prior_node.index) == 0:
            g.add_edge(g.vs[0], g.vs[-1])
...
class ChunkGenerator:
    ...
    def make_task_tree(self):
        ...
        task_tree.vs['start_ts'] = [self.task_start_ts.get(k, self.task_crt_ts[k]) for k in task_ids]
        task_tree.vs['end_ts'] = [self.task_end_ts[k] for k in task_ids]
        task_tree.vs['dur'] = [end - start for start, end in zip(task_tree.vs['start_ts'], task_tree.vs['end_ts'])]
    ...
    def __iter__(self):
        ...
        for location, event in self.events:
            ...
            # print(event)
            # print(event_defines_new_task_fragment(event, self.attr))
            # print(event.attributes)
            # print(chunk_stack)
            # print(len(chunk_stack))

            if self.verbose:
                print("EVENT: {} {} {}".format(event.time, type(event), event.attributes[self.attr["event_type"]]))
                print("  ATTRIBUTES: " + ", ".join([f"{attr.name}={event.attributes[attr]}" for attr in event.attributes]))
            ...
            if (type(event) == ThreadTaskCreate) or (type(event) == Enter and event.attributes[self.attr['region_type']] in [RegionType.initial_task, RegionType.implicit_task]):
                ...
                if unique_id not in self.task_start_ts:
                    self.task_start_ts[unique_id] = event.time
