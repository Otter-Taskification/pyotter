...
from otter.core.events import is_event_list, Event, Location
...
class OMPEventModel(BaseEventModel):
    ...
    def get_task_synchronisation_cache(self, event: Event) -> List[Task]:
        assert (event.region_type, event.event_type) in (
    ...
    @staticmethod
    def events_bridge_region(
        previous: Event, current: Event, region_types: List[RegionType]
    ) -> bool:
        # Used to check for certain enter-leave event sequences
        # assert is_event(previous) and is_event(current)
        return (
            previous.region_type in region_types
            and is_enter_event(previous)
            and current.region_type in region_types
            and is_leave_event(current)
        )

    @classmethod
    def events_bridge_single_master_region(
        cls, previous: Event, current: Event
    ) -> bool:
        return cls.events_bridge_region(
            previous,
            current,
            [RegionType.single_executor, RegionType.single_other, RegionType.master],
        )
    ...
    def check_events(
        ...
        reduce: Callable[[Iterable[bool]], bool],
    ...
    def all_single_exec_events(cls, events: Iterable[Event]):
        return cls.check_events(
            events, all, lambda event: event.region_type == RegionType.single_executor
        )

    @classmethod
    def all_master_events(cls, events: Iterable[Event]):
        return cls.check_events(
            events, all, lambda event: event.region_type == RegionType.master
        )

    @classmethod
    def all_taskwait_events(cls, events: Iterable[Event]):
        return cls.check_events(
            events, all, lambda event: event.region_type == RegionType.taskwait
        )
    ...
    def is_empty_task_region(cls, vertex: Vertex) -> bool:
        ...
        if vertex["_is_task_enter_node"] or vertex["_is_task_leave_node"]:
            return (
                vertex["_is_task_leave_node"] == True and vertex.indegree() == 0
            ) or (vertex["_is_task_enter_node"] == True and vertex.outdegree() == 0)
        # TODO: could this be refactored? Don't we already ensure that vertex['event_list'] is always a list?
        if type(vertex["event_list"]) is list and set(
            map(type, vertex["event_list"])
        ) in [{EventType.task_switch}]:
    ...
    def reject_task_create(self, events: List[Event]) -> List[Event]:
        events_filtered = [
            event for event in events if not self.is_task_create_event(event)
        ]
    ...
    def _return_unique_event(
        self, events: Union[EventList, List[EventList]], region_type: RegionType
    ):
        """
        ...
        if len(unique_events) == 1:
            ...
            self.log.debug(
                f"no {region_type} events, returning event list ({event_types=})"
            )
            result = events
        else:
    ...
    def return_unique_taskswitch_complete_event(self, events):
        ...
        event_set = set(
            event for event in events if self.is_task_switch_complete_event(event)
        )
    ...
    def get_augmented_event_attributes(event: Event) -> Dict:
        attr = event.to_dict()
        event_type, unique_id, region_type = (
            event.get(Attr.event_type),
            event.get(Attr.unique_id),
            event.get(Attr.region_type),
        )
...
def omp_chunk_to_graph(event_model: OMPEventModel, chunk: Chunk) -> Graph:
    ...
    master_enter_event = (
        chunk.first if chunk.first.region_type == RegionType.master else None
    )

    # Add attributes to the first node depending on chunk region type
    if chunk.type == RegionType.parallel:
        prior_vertex["_parallel_sequence_id"] = (
            chunk.first.unique_id,
            chunk.first.endpoint,
        )
    ...
    for event in islice(chunk.events, 1, None):
        ...
        if event.region_type in [
            RegionType.barrier_implicit,
            RegionType.barrier_explicit,
        ]:
            if is_enter_event(event):
                barrier_cluster_id = (
        ...
        if event.region_type == RegionType.taskwait:
            ...
            if is_enter_event(event):
                ...
                if chunk.type == RegionType.single_executor:
                    chunk.log.debug(
                        f"registering tasks at taskwait barrier inside a single-executor chunk"
                    )
                    barrier_context.synchronise_from(
                        event_model.get_task_synchronisation_cache(chunk.first)
                    )
                ...
                # Register tasks synchronised at a barrier
                    ...
                    barrier_context.synchronise_from(
                        encountering_task.task_barrier_cache
                    )
        ...
        if event.region_type == RegionType.single_executor:
            if is_enter_event(event):
                if encountering_task.has_active_task_group:
                    ...
                    group_context.synchronise_lazy(
                        event_model.get_task_synchronisation_cache(event)
                    )
                else:
                    # Record a reference to the cache to later add lazily to the next
                    # task-sync barrier this task encounters.
                    encountering_task.append_to_barrier_iterables_cache(
                        event_model.get_task_synchronisation_cache(event)
                    )
        ...
        if (
            chunk.type == RegionType.parallel
            and (is_enter_event(event) or is_leave_event(event))
            and event.region_type != RegionType.master
        ):
        ...
        if event.region_type == RegionType.parallel:
            v["_parallel_sequence_id"] = (
                chunk.first.unique_id if event is chunk.last else event.unique_id,
                event.endpoint,
            )
        ...
        events_bridge_single_master = event_model.events_bridge_single_master_region(
            prior_event, event
        )
        events_bridge_parallel = event_model.events_bridge_parallel_region(
            prior_event, event
        )
        events_have_same_id = (
            event.unique_id == prior_event.unique_id
            if events_bridge_parallel
            else False
        )
        if not (
            events_bridge_single_master
            or (events_bridge_parallel and events_have_same_id)
        ):
        ...
        if event_model.is_task_create_event(event):
            ...
            if encountering_task.has_active_task_group:
                ...
                if chunk.type == RegionType.single_executor:
                    chunk.log.debug(
                        f"registering new task in single-executor cache: parent={encountering_task.id}, child={created_task.id}"
                    )
                    event_model.get_task_synchronisation_cache(chunk.first).append(
                        created_task
                    )
    ...
    if graph.ecount() == 0:
        chunk.log.debug(
            f"graph contains no edges (type={chunk.type}, events={len(chunk)})"
        )
        if chunk.type == RegionType.explicit_task:
            ...
            chunk.log.debug(
                f"no internal vertices - add edge from: {graph.vs[0]['event_list']} to: {graph.vs[1]['event_list']}"
            )
    ...
    if (len(final_vertex.in_edges()) == 0) and (
        (chunk.type == RegionType.single_executor and len(chunk) > 2)
        or (chunk.type == RegionType.parallel and len(chunk) > 4)
    ):
...
def is_enter_event(event: Event) -> bool:
    return event.event_type in (
        EventType.thread_begin,
        EventType.parallel_begin,
        EventType.sync_begin,
        EventType.workshare_begin,
        EventType.phase_begin,
    )


def is_leave_event(event: Event) -> bool:
    return event.event_type in (
        EventType.thread_end,
        EventType.parallel_end,
        EventType.sync_end,
        EventType.workshare_end,
        EventType.phase_end,
    )


def reduce_by_parallel_sequence(
    event_model: OMPEventModel, reductions: ReductionDict, graph: Graph
) -> Graph:
    """
    ...
    labeller = SequenceLabeller(
        key_is_not_none("_parallel_sequence_id"), group_label="_parallel_sequence_id"
    )

    # When combining the event vertex attribute, prioritise single-executor over single-other
    reductions["event_list"] = LoggingValidatingReduction(
        event_model.return_unique_single_executor_event
    )
...
def reduce_by_single_exec_event(
    event_model: OMPEventModel, reductions: ReductionDict, graph: Graph
) -> Graph:
    """
    ...
    labeller = SequenceLabeller(
        lambda vtx: event_model.all_single_exec_events(vtx["event_list"]),
        group_label=lambda vtx: vtx["event_list"][0],
    )
...
def reduce_by_master_event(
    ...
    # NOT TESTED!
    labeller = SequenceLabeller(
        lambda vtx: event_model.all_master_events(vtx["event_list"]),
        group_label=lambda vtx: vtx["event_list"][0],
    )

    # When combining events, there should be exactly 1 unique master-begin/end event
    reductions["event_list"] = LoggingValidatingReduction(
        event_model.return_unique_master_event
    )
...
def remove_redundant_master_edges(
    event_model: OMPEventModel, reductions: ReductionDict, graph: Graph
) -> Graph:
    """
    ...
    *** This step assumes vertex['event_list'] is a bare event instead of an event list ***
    ...
    master_enter_vertices = filter(
        lambda vertex: event_model.is_event_type(
            vertex["event_list"], EventType.master_begin
        ),
        graph.vs,
    )
    master_leave_vertices = filter(
        lambda vertex: event_model.is_event_type(
            vertex["event_list"], EventType.master_end
        ),
        graph.vs,
    )
    master_enter_vertex_map = {
        enter_vertex["event_list"]: enter_vertex
        for enter_vertex in master_enter_vertices
    }
    master_vertex_pairs = (
        (master_enter_vertex_map[leave_vertex["_master_enter_event"]], leave_vertex)
        for leave_vertex in master_leave_vertices
    )
    neighbour_pairs = {
        (enter.predecessors()[0], leave.successors()[0])
        for enter, leave in master_vertex_pairs
    }
    redundant_edges = list(
        filter(
            lambda edge: (edge.source_vertex, edge.target_vertex) in neighbour_pairs,
            graph.es,
        )
    )
    event_model.log.info(
        f"deleting redundant edges due to master regions: {len(redundant_edges)}"
    )
    graph.delete_edges(redundant_edges)
    return graph


def reduce_by_task_cluster_id(
    event_model: OMPEventModel, reductions: ReductionDict, graph: Graph
) -> Graph:
    """
...
def reduce_by_task_id_for_empty_tasks(
    event_model: OMPEventModel, reductions: ReductionDict, graph: Graph
) -> Graph:
    """
    ...
    """
    event_model.log.info(
        "combining vertices by task ID where there are no nested nodes"
    )

    # Label vertices which represent empty tasks and have the same task ID
    labeller = SequenceLabeller(
        event_model.is_empty_task_region, group_label=lambda v: v["_task_cluster_id"][0]
    )

    # Combine _task_cluster_id tuples in a set (to remove duplicates)
    reductions["_task_cluster_id"] = LoggingValidatingReduction(
        handlers.pass_the_set_of_values,
        accept=tuple,
...
def reduce_by_sync_cluster_id(
    ...
    labeller = SequenceLabeller(
        key_is_not_none("_sync_cluster_id"), group_label="_sync_cluster_id"
    )
...
def combine_graphs(
    event_model: OMPEventModel, task_registry: TaskRegistry, graphs: Iterable[Graph]
) -> Graph:
    log = get_module_logger()
    ...
    for attribute, handler, accept in attribute_handlers:
        strategies[attribute] = LoggingValidatingReduction(
            handler, accept=accept, msg=f"combining attribute: {attribute}"
        )
    ...
    for task_sync_vertex in filter(
        lambda v: v["_task_sync_context"] is not None, graph.vs
    ):
        ...
        for synchronised_task in context:
            ...
            if context.synchronise_descendants:
                ...
                for descendant_task_id in task_registry.descendants_while(
                    synchronised_task.id, lambda task: not task.is_implicit()
                ):
                    descendant_task = task_registry[descendant_task_id]
                    # This task is synchronised by the context
                    edge = graph.add_edge(
                        descendant_task._dummy_vertex, task_sync_vertex
                    )
                    edge[Attr.edge_type] = edge_type
                    log.debug(
                        f"    ++ got synchronised descendant task {descendant_task_id}"
                    )
