...
from otter.core.tasks import Task, NullTask, TaskSynchronisationContext, TaskRegistry
from otter.utils import (
    SequenceLabeller,
    LoggingValidatingReduction,
    ReductionDict,
    handlers,
    transpose_list_to_dict,
)
...
class TaskGraphEventModel(BaseEventModel):
    ...
    def is_empty_task_region(cls, vertex: Vertex) -> bool:
        ...
        if vertex["_is_task_enter_node"] or vertex["_is_task_leave_node"]:
            return (
                vertex["_is_task_leave_node"] is True and vertex.indegree() == 0
            ) or (vertex["_is_task_enter_node"] is True and vertex.outdegree() == 0)
        ...
        raise RuntimeError("would return None where bool required")
...
def reduce_by_task_cluster_id(
    event_model: TaskGraphEventModel, reductions: ReductionDict, graph: Graph
) -> Graph:
    """
    ...
    reductions["event_list"] = LoggingValidatingReduction(
        handlers.pass_the_unique_value
    )
...
def combine_graphs(
    event_model: TaskGraphEventModel,
    task_registry: TaskRegistry,
    ...
    for name in vertex_attribute_names:
        if name not in graph.vs.attribute_names():
            log.warn(f"vertex attribute '{name}' not found, setting to 'None'")
            graph.vs[name] = None
    ...
    for task_sync_vertex in (
        v for v in graph.vs if v["_task_sync_context"] is not None
    ):
