...
colormap_task_flavour = {"0": "green", "1": "orange", "2": "blue", "3": "yellow"}
...
def style_graph(
    graph: Graph,
    style: graph_styling.GraphStylingProtocol = StyleVertexShapeAsRegionType(),
):
    per_vertex_styles_labels = [style.get_vertex_style(vertex) for vertex in graph.vs]
    vertex_styles, vertex_shapes, vertex_colors, vertex_labels = list(
        map(list, zip(*per_vertex_styles_labels))
    )
